  ...
  useEffect(() => {
    if (visible && branch) {
      setFormData({
        ...
        lng: branch.location?.coordinates?.[0]?.toString() || '106.6297',
        lat: branch.location?.coordinates?.[1]?.toString() || '10.8231'
      });
    } else if (visible && !branch) {
      setFormData({
        name: '',
        addressFull: '',
        addressCity: 'Hồ Chí Minh',
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditing && branch) {
        ...
        if (res.data?.success) {
          Alert.alert('Thành công', 'Đã cập nhật thông tin chi nhánh!');
          onSuccess();
          onClose();
        }
      } else {
