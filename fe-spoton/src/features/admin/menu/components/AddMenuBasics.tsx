  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        console.error(ADMIN_TEXTS.menu.basicsFetchError, error);
      }
    };
    fetchCategories();
  }, [category, setCategory]);
    ...
    <div className="flex-1 flex flex-col gap-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8">
        <div className="flex items-start gap-3 mb-8">
          ...
          <div>
            <h2 className="text-[17px] font-bold text-gray-900">{ADMIN_TEXTS.menu.basicsTitle}</h2>
            <p className="text-[13px] text-gray-500 mt-0.5">{ADMIN_TEXTS.menu.basicsDesc}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-6 mb-6">
          <div className="col-span-2">
            <label className="block text-[13px] font-bold text-gray-700 mb-1.5">
              {ADMIN_TEXTS.menu.basicsItemName} <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={itemName}
        ...
        <div className="mb-6">
          <label className="block text-[13px] font-bold text-gray-700 mb-1.5">
            {ADMIN_TEXTS.menu.basicsCategory} <span className="text-red-500">*</span>
          </label>
          <div className="relative">
            ...
            >
              <option value="">{ADMIN_TEXTS.menu.basicsCategorySelect}</option>
              {categories.map((c) => (
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label className="block text-[13px] font-bold text-gray-700">{ADMIN_TEXTS.menu.basicsDescription}</label>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 mb-24">
        <div className="flex items-start gap-3 mb-8">
          ...
          <div>
            <h2 className="text-[17px] font-bold text-gray-900">{ADMIN_TEXTS.menu.basicsDietaryTitle}</h2>
          </div>
        </div>

        <div>
          <label className="block text-[13px] font-bold text-gray-700 mb-3">{ADMIN_TEXTS.menu.basicsDietaryTags}</label>
          <div className="flex flex-wrap gap-2">
            <button className="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-full text-[13px] font-semibold text-gray-600 flex items-center gap-1.5 hover:bg-gray-100 transition-colors">
              ...
              {ADMIN_TEXTS.menu.basicsTagVegetarian}
            </button>
            <button className="px-3 py-1.5 bg-gray-50 border border-gray-200 rounded-full text-[13px] font-semibold text-gray-600 flex items-center gap-1.5 hover:bg-gray-100 transition-colors">
              <svg className="w-3.5 h-3.5 text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6" /></svg>
              {ADMIN_TEXTS.menu.basicsTagVegan}
            </button>
            <button className="px-3 py-1.5 bg-red-50 border border-red-200 rounded-full text-[13px] font-semibold text-red-600 flex items-center gap-1.5">
              ...
              {ADMIN_TEXTS.menu.basicsTagSpicy}
            </button>
            <button className="px-3 py-1.5 bg-[#fef3c7] border border-[#fde68a] rounded-full text-[13px] font-semibold text-[#b45309] flex items-center gap-1.5">
              ...
              {ADMIN_TEXTS.menu.basicsTagGluten}
            </button>
          </div>
        </div>
      </div>
