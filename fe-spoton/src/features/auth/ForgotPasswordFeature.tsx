...
export function ForgotPasswordFeature() {
  ...
  const onOtpSubmit = async (data: ForgotPasswordOtpSchema) => {
    // We don't verify OTP here because backend only has resetPassword endpoint which takes newPassword.
    // So we just save the OTP and go to step 3.
    // If the OTP is wrong, it will fail at step 3.
    setOtpCode(data.otp);
    setStep(3);
    setServerError(null);
