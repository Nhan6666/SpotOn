  ...
  if (isLoading) {
    return (
      <section className="bg-transparent py-16 text-center">
        <h2 className="text-[#F2B02A] text-4xl font-stencil tracking-tight mb-12 uppercase">{HOME_TEXTS.newDishes.title}</h2>
        <div className="flex justify-center">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-[#F2B02A]"></div>
        </div>
      </section>
    );
  }
  ...
  return (
    <section className="bg-transparent py-16 px-4 md:px-8 relative">
      <div className="max-w-7xl mx-auto">
        <h2 className="text-[#F2B02A] text-[42px] font-stencil tracking-tight mb-12 text-center uppercase">{HOME_TEXTS.newDishes.title}</h2>
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 min-h-[600px]">
            {currentItems.map((item) => (
              <div key={item._id} className="bg-[#0A2A12] border border-[#2A5A3A] rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-shadow group h-full flex flex-col cursor-pointer">
                <div className="h-[180px] w-full overflow-hidden relative shrink-0">
                  {item.image_url ? (
                    ...
                    <div className="w-full h-full bg-[#164626] flex items-center justify-center text-gray-400">
                ...
                <div className="p-5 flex flex-col flex-1 justify-between">
                  <div>
                    <h3 className="text-[17px] font-bold text-white line-clamp-2 mb-1">{item.name}</h3>
                    <p className="text-[#F2B02A] font-semibold text-[15px]">
                      {(item.base_price || 0).toLocaleString('vi-VN')} {HOME_TEXTS.newDishes.currency}
                    </p>
                  </div>
                  <div className="mt-4 flex justify-end">
                    <Link href="/branches" className="flex items-center gap-1.5 px-4 py-1.5 rounded-full border border-[#2A5A3A] hover:border-[#F2B02A] hover:text-[#F2B02A] hover:bg-[#164626] transition-colors text-sm font-semibold text-gray-300 cursor-pointer">
          ...
          <div className="flex justify-center gap-2 mt-10">
            {Array.from({ length: totalPages }).map((_, idx) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-colors cursor-pointer ${idx === currentPage ? 'bg-[#F2B02A]' : 'bg-white/20 hover:bg-white/40'}`}
        ...
        <div className="flex justify-center mt-10">
          <Link href="/menu" className="px-8 py-3 rounded-lg border-2 border-[#F2B02A] text-[#F2B02A] font-bold text-[15px] hover:bg-[#F2B02A] hover:text-[#164626] transition-colors inline-block uppercase tracking-wide">
