...
import { Users, DollarSign, Receipt, AlertTriangle, TrendingUp, AlertCircle, MessageSquareWarning } from 'lucide-react';
...
export function ManagerStatsFeature() {
  const { user } = useAuth();
  ...
  const fetchStats = async () => {
    ...
    try {
      setIsLoading(true);
      const data = await http.get<{ success: boolean; data: DashboardStats }>(
        `/stats/branch/${user.branch_id}/dashboard?startDate=${startDate.toISOString()}&endDate=${endDate.toISOString()}`
      );
      if (data.success) {
        setStats(data.data);
      }
      ...
      toastError("Lỗi kết nối khi tải dữ liệu thống kê.");
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleReopen = async () => {
    ...
    setIsHandling(true);
    try {
      await http.put(`/branches/${user.branch_id}`, { status: 'OPEN' });
      await fetchStats();
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex items-center gap-4">
          <select 
            ...
            className="px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 focus:ring-[#ef5914] focus:border-transparent text-gray-700"
          >
          ...
          {stats?.isOverloaded && (
            <div className="flex items-center gap-2 px-4 py-2 bg-red-100 text-red-700 rounded-lg font-semibold animate-pulse shadow-sm">
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-start justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 mb-1">Số đơn hàng</p>
          ...
          <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 flex items-start justify-between">
            <div>
              <p className="text-sm font-medium text-gray-500 mb-1">Review chờ xử lý</p>
              <h3 className={`text-2xl font-bold ${(stats?.pendingFeedback || 0) > 0 ? 'text-orange-600' : 'text-gray-900'}`}>
                {stats?.pendingFeedback || 0}
              </h3>
            </div>
            <div className="p-3 bg-orange-50 rounded-xl">
              <MessageSquareWarning className={`w-6 h-6 ${(stats?.pendingFeedback || 0) > 0 ? 'text-orange-600' : 'text-gray-400'}`} />
            </div>
          </div>
          ...
          <div className={`bg-white rounded-2xl p-6 shadow-sm border ${stats?.isOverloaded ? 'border-red-300' : 'border-gray-100'}`}>
            ...
            {stats?.isOverloaded && stats?.branchStatus !== 'FULL' && (
              <button 
                ...
                disabled={isHandling}
                className="w-full mt-2 py-2 bg-red-600 hover:bg-red-700 text-white text-sm font-medium rounded-lg transition-colors flex items-center justify-center gap-2 shadow-sm disabled:opacity-50"
              >
                <AlertTriangle className="w-4 h-4" /> Xử lý (Set FULL)
              </button>
            )}
            ...
            {stats?.branchStatus === 'FULL' && (
              <button 
                ...
                disabled={isHandling}
                className="w-full mt-2 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg transition-colors shadow-sm disabled:opacity-50"
              >
                Mở nhận khách lại
              </button>
            )}
      ...
      <Modal isOpen={showConfirmModal} onClose={() => setShowConfirmModal(false)} maxWidth="sm">
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 text-center mb-6">
            Chi nhánh của bạn sẽ được chuyển sang trạng thái <b>FULL</b>. Mọi kênh đặt bàn mới sẽ bị vô hiệu hóa tạm thời.
          </p>
          <div className="flex gap-3">
            <button 
              onClick={() => setShowConfirmModal(false)}
              className="flex-1 px-4 py-2 bg-gray-100 text-gray-700 hover:bg-gray-200 font-medium rounded-lg"
              disabled={isHandling}
            >
              Hủy
            </button>
            <button 
              ...
              disabled={isHandling}
              className="flex-1 px-4 py-2 bg-red-600 text-white hover:bg-red-700 font-medium rounded-lg"
            >
      ...
      <Modal isOpen={showReopenModal} onClose={() => setShowReopenModal(false)} maxWidth="sm">
        <div className="p-6">
          ...
          <p className="text-sm text-gray-500 text-center mb-6">
            Chi nhánh sẽ được chuyển về trạng thái <b>Hoạt động (OPEN)</b> và nhận khách trở lại.
          </p>
          <div className="flex gap-3">
            <button 
            ...
            <button 
              ...
              disabled={isHandling}
              className="flex-1 px-4 py-2 bg-green-600 text-white hover:bg-green-700 font-medium rounded-lg"
            >
