...
export function BranchesFeature() {
  ...
  const getTotalCapacity = (branch: PublicBranch) => {
    ...
    const total = branch.zones.reduce((sum, zone) => {
      const zoneCap = zone.tables?.reduce((tSum, t) => tSum + (t.capacity || 0), 0) || 0;
      return sum + zoneCap;
    }, 0);
      ...
      <div className="bg-white border-b border-gray-100 py-8">
        <div className="container mx-auto px-4 max-w-6xl">
          ...
          <div className="flex items-center text-sm text-gray-500 mb-4">
            ...
            <span className="text-gray-900 font-medium">Chi nhánh tại Cần Thơ</span>
          </div>

          <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">
            {isLoading ? 'Đang tải...' : `${branches.length} Chi nhánh`}
          </h1>
          <p className="text-gray-500">Danh sách các chi nhánh SpotOn đang hoạt động.</p>
        </div>
      </div>
          ...
          <div className="w-full lg:w-1/4">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-5 sticky top-24">
              <div className="flex items-center justify-between mb-6 border-b border-gray-100 pb-4">
                ...
                <button className="text-sm text-[#ea580c] hover:underline font-medium">Xóa tất cả</button>
              </div>
              ...
              {/* Area Preferences */}
              <div className="mb-6">
                <label className="block text-sm font-bold text-gray-700 mb-3">Khu vực ưa thích</label>
                <div className="space-y-3">
                  {['Máy lạnh', 'Sân vườn', 'Phòng riêng', 'Khu hút thuốc'].map((item) => (
          ...
          <div className="w-full lg:w-3/4 flex flex-col gap-6">
            ...
            {isLoading && (
              <div className="flex flex-col items-center justify-center py-20">
                ...
                <p className="text-gray-500 font-medium">Đang tải danh sách chi nhánh...</p>
              </div>
            )}
            ...
            {!isLoading && !error && branches.length === 0 && (
              <div className="bg-white rounded-xl border border-gray-200 p-12 text-center">
                ...
                <h3 className="text-lg font-bold text-gray-700 mb-1">Chưa có chi nhánh nào</h3>
                <p className="text-gray-500 text-sm">Hệ thống chưa có chi nhánh nào được tạo.</p>
              </div>
            )}
            ...
            {!isLoading && branches.map((branch, index) => {
              ...
              return (
                <div key={branch._id} className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col md:flex-row hover:shadow-md transition-shadow group">
                  ...
                  <div className="w-full md:w-5/12 h-56 md:h-auto relative overflow-hidden flex-shrink-0">
                    ...
                    />
                    {/* Status Badge */}
                    <div className={`absolute top-4 left-4 ${statusInfo.bg} border ${statusInfo.color} text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded shadow-sm z-10`}>
                      {statusInfo.label}
                    </div>
                  </div>
                  ...
                  <div className="w-full md:w-7/12 p-5 lg:p-6 flex flex-col">
                    ...
                    <div className="grid grid-cols-2 gap-y-3 gap-x-4 mb-5">
                      ...
                      <div className="flex items-center text-sm text-gray-600">
                        ...
                        <span className="truncate">{getDistrictDisplay(branch)}</span>
                      </div>
                      ...
                      {totalCapacity && (
                        <div className="flex items-center text-sm text-gray-600">
                          ...
                          <span className="truncate">Sức chứa: {totalCapacity} khách</span>
                        </div>
                      )}
                      ...
                      {branch.zones && branch.zones.length > 0 && (
                        <div className="flex items-center text-sm text-gray-600">
                          ...
                          <span className="truncate">{branch.zones.length} khu vực</span>
                        </div>
                      )}
                    </div>
                    ...
                    {serviceHours && (
                      <div className="bg-gray-50 rounded-lg px-4 py-3 mb-5">
                        <div className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wider mb-2">
                          <svg className="w-3.5 h-3.5 text-[#ea580c]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                          Giờ phục vụ
                        </div>
                        <div className="flex flex-wrap gap-x-6 gap-y-1">
                          {serviceHours.map((h, i) => (
                            <span key={i} className="text-sm text-gray-700 font-medium">{h}</span>
                          ))}
                        </div>
                      </div>
                    )}
                    ...
                    <div className="mt-auto flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div className="flex flex-wrap gap-2">
                        ...
                        <span className={`text-[11px] font-medium px-2.5 py-1 rounded border ${statusInfo.bg} ${statusInfo.color}`}>
                          {statusInfo.label}
                        </span>
                      ...
                      <Link href={`/branches/${branch._id}?date=${searchParams.get('date') || ''}&time=${searchParams.get('time') || ''}&guests=${searchParams.get('guests') || ''}`} className="w-full sm:w-auto px-6 py-2.5 border-2 border-[#ea580c] text-[#ea580c] hover:bg-[#fff6f0] font-bold rounded-full text-sm transition-colors text-center shrink-0">
                        Xem chi tiết
                      </Link>
                    </div>
                  </div>
